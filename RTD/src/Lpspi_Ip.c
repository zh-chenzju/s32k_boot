...
#define LPSPI_IP_SW_MAJOR_VERSION_C               3
...
#ifdef ERR_IPV_LPSPIV2_0001_2ND_SOLUTION
...
#endif
#if (STD_ON == LPSPI_IP_ENABLE_DMA_CONT_MEM_TRANSFER_SUPPORT)
...
#ifdef ERR_IPV_LPSPIV2_0001
static void Lpspi_ErrataHandle(LPSPI_Type* Base, Lpspi_Ip_StateStructureType* State);
#endif
/*==================================================================================================
*                                      LOCAL FUNCTIONS
==================================================================================================*/
...
static void Lpspi_Ip_DmaInit(uint8 Instance)
{
    ...
    #endif
    /* Init default value for some channel TCD status. These variables will be update in time of configuring TX/RX DMA for each channel to save current attributes of that channel */
...
static void Lpspi_Ip_TxDmaConfig(uint8 Instance)
{
    ...
    if (NumberDmaIterWrite != State->PreTxMajorLoopCount)
    {
        ...
        /* At next channel, if PreTxMajorLoopCount is same current channel, do not need re-configure DMA_IP_CH_SET_MAJORLOOP_COUNT to increase the performance */
...
static void Lpspi_Ip_RxDmaConfig(uint8 Instance)
{
    ...
    if (NumberDmaIterRead != State->PreRxMajorLoopCount)
    {
        ...
        DmaTcdListIndexCounter++;
        /* At next channel, if PreRxMajorLoopCount is same current channel, do not need re-configure DMA_IP_CH_SET_MAJORLOOP_COUNT to increase the performance */
...
#if ((STD_ON == LPSPI_IP_DMA_USED) && (STD_ON == LPSPI_IP_ENABLE_DMA_CONT_MEM_TRANSFER_SUPPORT))
static void Lpspi_TransmitTxDmaContMemReconfig(uint8 Instance,
                                 const uint8* TxBuffer,
                                 uint16 NumberOfFrames
                                )
...
#if ((STD_ON == LPSPI_IP_ENABLE_DMA_CONT_MEM_TRANSFER_SUPPORT) && (STD_ON == LPSPI_IP_DMA_USED))
Lpspi_Ip_StatusType Lpspi_Ip_DmaContMemAsyncTransmit(
                                            const Lpspi_Ip_ExternalDeviceType *ExternalDevice,
...
void Lpspi_Ip_Cancel(uint8 Instance)
{
    ...
    if (LPSPI_IP_BUSY == State->Status)
    {
        ...
        if (TRUE == State->PhyUnitConfig->DmaUsed)
        {
            ...
            (void)Dma_Ip_SetLogicChannelCommand(State->PhyUnitConfig->RxDmaChannel, DMA_IP_CH_CLEAR_HARDWARE_REQUEST);
            (void)Dma_Ip_SetLogicChannelCommand(State->PhyUnitConfig->TxDmaChannel, DMA_IP_CH_CLEAR_HARDWARE_REQUEST);
            /* Reset value for some channel TCD status. These variables will be updated in time of configuring TX/RX DMA for each channel to save current attributes of that channel */
            State->IsPreTxBufferNull = FALSE;
            State->IsPreRxBufferNull = FALSE;
            State->PreTxMajorLoopCount = 0u;
            State->PreRxMajorLoopCount = 0u;
